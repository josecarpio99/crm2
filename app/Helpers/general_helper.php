...
if (!function_exists('send_app_mail')) {
    ...
    function send_app_mail($to, $subject, $message, $optoins = array(), $convert_message_to_html = true) {
        ...
        if (get_setting("email_protocol") === "smtp") {
            ...
            $email_config["SMTPHost"] = $is_user_smtp_setup ? $login_user->email_smtp_host : get_setting("email_smtp_host");
            $email_config["SMTPPort"] = $is_user_smtp_setup ? $login_user->email_smtp_port : get_setting("email_smtp_port");
            $email_config["SMTPUser"] = $is_user_smtp_setup ? $login_user->email_smtp_user : get_setting("email_smtp_user");
            $email_config["SMTPPass"] = $is_user_smtp_setup 
                                        ? decode_password($login_user->email_smtp_password, "email_smtp_pass")
                                        : decode_password(get_setting('email_smtp_pass'), "email_smtp_pass");
            $email_config["SMTPCrypto"] = $is_user_smtp_setup ? $login_user->email_smtp_security_type : get_setting("email_smtp_security_type");
